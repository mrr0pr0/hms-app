...
export default async function handler(
  ...
  // denne API-ruten håndterer både innlogging og registrering av brukere, samt henting av brukerdata basert på hash
  try {
    await initDB();
    ...
    if (req.method === "PUT") {
      // bruker PUT for registrering siden POST allerede brukes for login, dette kan endres
      const { username, password } = req.body; // hent username og password fra request body
      if (!username || !password)
        return res.status(400).json({ error: "Missing username or password" }); // username og password er påkrevd
