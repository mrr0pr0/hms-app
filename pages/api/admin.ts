...
export default async function handler(req: NextApiRequest, res: NextApiResponse) { // gir amdmin en kode for å oppgradere seg selv til admin, for testing
  ...
  try { 
    ...
    if (!hash || !code) return res.status(400).json({ error: 'Missing hash or code' }) // hvis en manger så returner 400

    if (!ADMIN_CODES.includes(code.trim().toUpperCase())) { // hvis koden ikke er gyldig så returner 403
