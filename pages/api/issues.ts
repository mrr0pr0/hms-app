...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    if (req.method === 'POST') { 
      ...
      if (!title || !created_by) return res.status(400).json({ error: 'Missing title or created_by' }) // title og created_by er påkrevd
    ...
    if (req.method === 'PATCH') {
      ...
      const users = await sql`SELECT is_admin FROM users WHERE hash = ${hash} LIMIT 1` // hent is_admin for brukeren med gitt hash
      if (!users.length || !users[0].is_admin) { // hvis ingen bruker funnet eller ikke admin så returner 403
      ...
      if (status !== undefined) {
        if (!validStatuses.includes(status)) return res.status(400).json({ error: 'Invalid status' }) // hvis status er gitt så må den være gyldig
